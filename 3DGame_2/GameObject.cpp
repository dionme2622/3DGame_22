...
void CRotatingObject::Animate(float fElapsedTime, XMFLOAT3 pos)
{
	SetPosition(pos);
...
CEnemyObject::CEnemyObject(CPlayer* pPlayer)
{
	m_player = pPlayer;
	...
	for (int i = 0; i < BULLETS; i++)
	{
		...
		m_ppBullets[i]->SetRotationAxis(XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_ppBullets[i]->SetRotationSpeed(360.0f);
		m_ppBullets[i]->SetMovingSpeed(25.0f);
		m_ppBullets[i]->SetActive(false);
	}
}
...
{
	m_cooltime += fElapsedTime;
	if (m_cooltime > 5.0) {		// 5초가 지나면 총알을 발사한다
		FireBullet();		// player 객체를 넘겨야함
		m_cooltime = 0.0f;		//  다시 쿨타임을 돌린다.
	}
	for (int i = 0; i < BULLETS; i++)
	{
...
void CEnemyObject::FireBullet()	// 적이 총알 발사
{
	...
	if (pBulletObject)
	{
		XMFLOAT3 xmf3Position = GetPosition();		// 총알의 위치 벡터
		XMFLOAT3 xmf3Direction = ((CGameObject*)m_player)->GetPosition();		// 플레이어의 위치 벡터를
		XMFLOAT3 xmf3movingDirection = Vector3::Subtract(xmf3Direction, xmf3Position); // 플레이어 위치벡터와 총알의 위치벡터를 뺀다
		XMFLOAT3 xmf3FirePosition = Vector3::Add(xmf3Position, Vector3::ScalarProduct(m_xmf3MovingDirection, 6.0f, false));

		pBulletObject->m_xmf4x4World = m_xmf4x4World;

		pBulletObject->SetFirePosition(xmf3FirePosition);
		pBulletObject->SetMovingDirection(xmf3movingDirection);
