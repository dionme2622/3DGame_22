...
CAirplanePlayer::CAirplanePlayer()
{
	...
	CCubeMesh* pBarrierMesh = new CCubeMesh(6.0f, 6.0f, 6.0f);	// 배리어를 그린다
	m_barrier = new CRotatingObject();
	m_barrier ->SetMesh(pBarrierMesh);
	m_barrier ->SetColor(RGB(255, 0, 255));
	m_barrier ->SetRotationAxis(XMFLOAT3(0.0f, 1.0f, 1.0f));
	m_barrier ->SetRotationSpeed(360.0f);
...
void CAirplanePlayer::Animate(float fElapsedTime)
{
	...
	if (m_barrier_swtich) {
		m_barrier->Animate(fElapsedTime,GetPosition());
	}
	switch (m_hp)
	{
	case 3 :
...
void CAirplanePlayer::Render(HDC hDCFrameBuffer, CCamera* pCamera)
{
	...
	if (m_barrier_swtich) m_barrier->Render(hDCFrameBuffer, pCamera);	// 배리어를 그린다.
}
